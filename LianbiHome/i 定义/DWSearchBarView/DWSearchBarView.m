    ...
    for (int i = 0; i <= (self.upDatasArrays.count - 1) / 3; i++) {
        for (int j = 0 ; j < 3; j++) {
            ...
            if (index < self.upDatasArrays.count) {
                UIButton * btn = [[UIButton alloc]initWithFrame:CGRectMake(x + j * (b_w +20), hei *  i + 10 + hei, b_w, hei - 20)];
                ...
                [[btn titleLabel]setFont:[UIFont systemFontOfSize:14.5]];
...
-(void)createDownView
{
    ...
    for (int i = 0 ; i < downArray.count; i++) {
        ...
        label.font = [UIFont systemFontOfSize:JN_HH(13.5)];
